...
@CommandPkg(
        ...
        label = "Allergy Lookup",
        node_label = "Read allergy intolerance in FHIR session {{sessionName}}",
        group_label = "Allergy",
        description = "Returns Allergy Details",
        icon = "fhir2.svg",
        comment = true ,
        //text_color = "#7B848B" ,
        return_label = "Assign output to a dictionary",
        return_type = DataType.DICTIONARY,
        return_description = "Keys for dictionary: 'description', 'catergory', 'patient', 'onset', 'reaction'")

public class ReadAllergy {
    @Sessions
    ...
    public DictionaryValue action(
            ...
            @Idx(index = "3", type = TEXT) @Pkg(label = "FHIR ID", description = "The FHIR Allergy Intolerance ID")
        ...
        Map<String, Value> resMap = FHIRActions.readAllergyIntolerance(url, "Bearer " + access_token, fhirId);
