...
@CommandPkg(
        ...
        node_label = "Book Appointment in FHIR session {{sessionName}}",
        ...
        return_description = "Keys for dictionary: 'id', 'start', 'end', 'status', 'minutesDuration', " +
                "'created', 'serviceType'")
...
public class BookAppointment {
    ...
    public DictionaryValue action(
            ...
            @Pkg(label = "Read Coverage", description = "The FHIR Appointment $book operation schedules an appointment that was returned by the $find operation.")
                    String help,
            @Idx(index = "2", type = TEXT) @Pkg(label = "Session name", default_value_type = STRING, default_value = "Default")
            @NotEmpty String sessionName,
            @Idx(index = "3", type = TEXT) @Pkg(label = "FHIR ID", description = "The FHIR Appointment ID obtained from Appointment.$find")
        ...
        DictionaryValue appt = FHIRActions.bookAppointment(url, "Bearer " + access_token, fhirId);
        return appt;
    }
